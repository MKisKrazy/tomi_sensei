...
def calculate_portfolio_allocation(
    ...
    """Calculates rupees amounts from percentage allocations.

    Args:
        budget: Total investment budget in INR.
        allocations_json: JSON string mapping tickers to percentages.
            Example: '{"AAPL": 40, "GOOGL": 35, "BND": 25}'

    Returns:
        JSON with per-position rupees amounts and validation.
    """
    try:
    ...
    for ticker, pct in allocations.items():
        result[ticker] = {
            "percentage": pct,
            "ruppes_amount": round(budget * pct / 100, 2),
        }
